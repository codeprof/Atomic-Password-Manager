...
//EDIT:
QString encode(QString text, QString str_search, QString str_replace)
{
	QString newtext = "";
...
void DatabaseWidget::copyPasswordEnc()
{
	QString search = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_#!+?$%~";
    QString replace = "8a~$6uJ+Vdjgx01DozBO2fs4SvXF_#ArH59MTY%ipqnQRWew!lyZGULm7tCEhKP?3NIcbk";
    Entry* currentEntry = m_entryView->currentEntry();
    if (!currentEntry) {
        Q_ASSERT(false);
        return;
    ...
    QString result = "^";
    result = result.append(encode(currentEntry->password(),replace, search));
    setClipboardTextAndMinimize(result);
}
